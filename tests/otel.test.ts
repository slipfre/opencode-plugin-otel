...
describe("buildResource", () => {
  ...
  test("includes service metadata", () => {
    delete process.env["OTEL_RESOURCE_ATTRIBUTES"];
    const resource = buildResource("1.14.20");
    const attrs = resource.attributes;
    expect(attrs["service.name"]).toBe("opencode");
    expect(attrs["service.version"]).toBe("1.14.20");
    expect(attrs["opencode.plugin.version"]).toBeUndefined();
    expect(attrs["app.version"]).toBeUndefined();
    expect(attrs["os.type"]).toBe(process.platform);
    expect(attrs["host.arch"]).toBe(process.arch);
  });

  test("uses unknown when the service version is unavailable", () => {
    delete process.env["OTEL_RESOURCE_ATTRIBUTES"];
    expect(buildResource().attributes["service.version"]).toBe("unknown");
  });

  test("merges OTEL_RESOURCE_ATTRIBUTES from env", () => {
    process.env["OTEL_RESOURCE_ATTRIBUTES"] = "team=platform,env=prod";
    const attrs = buildResource("1.14.20").attributes;
...
describe("setupOtel", () => {
  ...
  test("uses the gRPC exporter for grpc", async () => {
    providers = await setupOtel("http://collector:4317", "grpc", "1.14.20");
