...
import type { EventMessagePartUpdated, EventSessionIdle } from "@opencode-ai/sdk"
...
function makeToolRunning(sessionID = "ses_1", callID = "call_1"): EventMessagePartUpdated {
  return {
    ...
    properties: {
      part: {
        ...
        callID,
        tool: "bash",
        state: { status: "running", input: { command: "pwd" }, time: { start: 1000 } },
      },
    },
  } as unknown as EventMessagePartUpdated
}

function makeToolCompleted(sessionID = "ses_1", callID = "call_1"): EventMessagePartUpdated {
  return {
    type: "message.part.updated",
    properties: {
      part: {
...
function makeToolError(sessionID = "ses_1", callID = "call_1"): EventMessagePartUpdated {
  return {
    type: "message.part.updated",
    properties: {
      part: {
...
function makePermissionAsked(overrides: {
  id?: string
  sessionID?: string
  permission?: string
  patterns?: string[]
...
describe("permission spans", () => {
  test("creates a guardrail child span for the matching active tool", async () => {
    const { ctx, tracer } = makeCtx("proj_test", { "deployment.environment": "test" })
    ...
    expect(permission.attributes[OPENINFERENCE_SPAN_KIND]).toBe(OpenInferenceSpanKind.GUARDRAIL)
  ...
  test("ends a granted permission span with its reply and wait duration", async () => {
    ...
    expect(permission.attributes["permission.wait_ms"]).toBeGreaterThanOrEqual(0)
  ...
  test("does not fall back to an interaction when the request has no tool", async () => {
    ...
    await permissionHandlers.asked(makePermissionAsked({ permission: "doom_loop", tool: null }), ctx)
  ...
  test("does not create a span when the referenced tool span is missing", async () => {
    ...
    expect(pluginLog.calls.at(-1)).toMatchObject({
      ...
      extra: { sessionID: "ses_1", requestID: "per_1", callID: "call_1", permission: "bash" },
    })
  })
  ...
  test("ignores duplicate requests without creating another span", async () => {
    ...
    expect(tracer.spans.filter(span => span.name === "opencode.permission.check")).toHaveLength(1)
    expect(pluginLog.calls.at(-1)).toMatchObject({
      level: "debug",
  ...
  test("ends the oldest active span when the correlation map reaches capacity", async () => {
    ...
    for (let index = 0; index <= MAX_PENDING; index += 1) {
      await permissionHandlers.asked(makePermissionAsked({ id: `per_${index}` }), ctx)
    }

    const permissions = tracer.spans.filter(span => span.name === "opencode.permission.check")
