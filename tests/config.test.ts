...
describe("parseAttributePairs", () => {
  ...
  test("trims whitespace and preserves equals signs in values", () => {
    expect(parseAttributePairs(" team = platform , auth = Bearer abc=123 ")).toEqual({
      team: "platform",
      auth: "Bearer abc=123",
    })
  })
  ...
  test("ignores malformed pairs", () => {
    expect(parseAttributePairs("good=value,bad,=missing")).toEqual({ good: "value" })
  })
...
describe("loadConfig", () => {
  ...
  test("reads endpoint, protocol, and trace prefix", () => {
    process.env["OPENCODE_OTLP_ENDPOINT"] = "https://collector.example.com/otlp"
  ...
  test("reads trace parent settings", () => {
    process.env["OPENCODE_TRACEPARENT"] = "00-0123456789abcdef0123456789abcdef-0123456789abcdef-01"
  ...
  test("copies headers and resource attributes to OTel environment variables", () => {
    ...
    expect(process.env["OTEL_EXPORTER_OTLP_HEADERS"]).toBe("Authorization=Bearer token")
  ...
  test("reads user ID environment settings", () => {
    ...
    process.env["OPENCODE_USER_ID_ENDPOINT"] = "https://identity.example.com/query"
...
describe("resolveHelperPath", () => {
  test("replaces project placeholders", () => {
    expect(resolveHelperPath("${PROJECT_ROOT}/helper", "/directory", "/worktree")).toBe("/worktree/helper")
    expect(resolveHelperPath("${WORKTREE}/helper", "/directory", undefined)).toBe("/directory/helper")
    expect(resolveHelperPath("${DIRECTORY}/helper", "/directory", "/worktree")).toBe("/directory/helper")
  })

  test("returns undefined unchanged", () => {
    expect(resolveHelperPath(undefined, "/directory", "/worktree")).toBeUndefined()
  })
})
