...
describe("user ID resolution", () => {
  test("posts the API key token to the configured endpoint", async () => {
    ...
    expect(requests[0]?.input).toBe(
      "https://identity.example.com/queryUserByToken"
    );
    ...
    expect(
      logs.filter((entry) => entry.message.startsWith("user ID request"))
    ).toEqual([
      {
        level: "debug",
  ...
  test("retries with backoff before returning unknown", async () => {
    ...
    expect(calls).toBe(2);
    expect(
      logs.filter((entry) => entry.message === "user ID request sent")
    ).toHaveLength(2);
    expect(
      logs
        .filter((entry) => entry.message === "user ID request failed")
  ...
  test("uses the configured retry count with exponential backoff", async () => {
    ...
    const resolver = createTestResolver({
      ...
      log: async (_level, message, extra) => {
        if (
          message === "user ID request failed" &&
          typeof extra?.retryDelayMs === "number"
        ) {
