...
Run spans include `opencode.run.id`, use the OpenInference `CHAIN` kind, expose interaction inputs as a JSON array, and use the final interaction output as the run output. Interaction spans include `opencode.interaction.id` and use the OpenInference `AGENT` kind. Compaction spans use the OpenInference `CHAIN` kind and include `opencode.compaction.id`, `opencode.compaction.auto`, and `opencode.compaction.overflow`. When summary usage is available, compaction spans also include `opencode.compaction.prompt_tokens` and `opencode.compaction.summary_tokens`. Overflow compactions include `opencode.compaction.trigger_message.id`. Their summary LLM spans include `opencode.llm.purpose=compaction`. LLM and tool spans include model, token, cost, input, output, status, and timing attributes when available. LLM spans include `opencode.llm.retry_count`, initialized to `0` and updated from OpenCode retry status events. Manual permission checks create OpenInference `GUARDRAIL` spans only when the request identifies an active tool span. They include the request, permission, tool, reply, grant result, and wait duration; uncorrelated checks are logged and do not fall back to interaction or run parents.
