...
function active(
  ...
  const correlated =
    input.metadata &&
    ctx.llmTelemetryBindings.byLifecycleMetadata.get(input.metadata);
  if (correlated) {
    return correlated;
  }
  if (!input.headers) {
...
function cleanContentPart(value: unknown): CleanContent | undefined {
  ...
  const file =
    value.type === "file" && isRecord(value.file) ? value.file : value;
  if (
    value.type === "image" ||
    (value.type === "file" &&
    ...
    const url = imageUrl(
      value.image ?? file.data ?? file.base64 ?? file.uint8Array ?? file.url,
      file.mediaType
    );
    if (url) {
...
function cleanMessages(values: unknown[]): CleanMessage[] {
  ...
  for (const value of values) {
    ...
    const content = value.content
      .map(cleanContentPart)
      .filter((part): part is CleanContent => part !== undefined);
    if (content.length > 0) {
...
function outputMessages(event: OnStepFinishEvent): CleanMessage[] {
  const messages = cleanMessages(event.response.messages).filter(
    (message) => message.role === "assistant"
  );
  const generated = event.content
    .map(cleanContentPart)
    .filter((part): part is CleanContent => part !== undefined);
...
function messageAttributes(
  ...
  messages.forEach((message, messageIndex) => {
    ...
    if (message.toolCallId) {
      attributes[`${messagePrefix}.${MESSAGE_TOOL_CALL_ID}`] =
        message.toolCallId;
    }
    ...
    message.content.forEach((content, contentIndex) => {
      ...
      if (content.type === "text" || content.type === "reasoning") {
        ...
        attributes[`${contentPrefix}.${MESSAGE_CONTENT_IMAGE}.${IMAGE_URL}`] =
          content.imageUrl;
        ...
        if (content.name) {
          attributes[`${contentPrefix}.${TOOL_CALL_FUNCTION_NAME}`] =
            content.name;
        }
        if (content.arguments !== undefined) {
          attributes[`${contentPrefix}.${TOOL_CALL_FUNCTION_ARGUMENTS_JSON}`] =
            json(content.arguments);
        }
      }
    });
  });
...
function headerAttribute(
  key: "http.request.headers" | "http.response.headers",
...
function inputSchema(value: unknown): unknown {
  return isRecord(value) && value.jsonSchema !== undefined
    ? value.jsonSchema
    : value;
}
...
function toolAttributes(event: OnStepStartEvent): Attributes {
  ...
  const activeTools = event.activeTools
    ? new Set<string>(event.activeTools)
    : undefined;
  const tools = Object.entries(event.tools).filter(
    ([name]) => !activeTools || activeTools.has(name)
  );
...
function handleAiTelemetryStart(event: OnStartEvent, ctx: HandlerContext) {
  ...
  current.span.setAttribute(
    LLM_INVOCATION_PARAMETERS,
    json(invocationParameters(event))
  );
}

function handleAiTelemetryStepStart(
...
function handleAiTelemetryStepFinish(
