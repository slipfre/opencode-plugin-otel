...
const compactionHandlers = {
  ...
  start(part: RuntimeCompactionPart, ctx: HandlerContext) {
    ...
    const startTime =
      user?.sessionID === part.sessionID ? user.startTime : Date.now();
    const run =
      ctx.activeRunSpans.get(part.sessionID) ??
      ensureRunStarted(
        part.sessionID,
          ...
          ...(overflow && pendingOverflow
            ? {
                "opencode.compaction.trigger_message.id":
                  pendingOverflow.messageID,
              }
            : {}),
          ...(ownerInteractionID
            ? { "opencode.interaction.id": ownerInteractionID }
            : {}),
          ...ctx.commonAttrs,
        },
      },
    ...
    const record = {
      ...
      overflow,
      ...(overflow && pendingOverflow
        ? { triggerMessageID: pendingOverflow.messageID }
        : {}),
  ...
  handlePart(part: Part, ctx: HandlerContext) {
    ...
    const compaction =
      ctx.activeCompactions.get(part.sessionID) ??
