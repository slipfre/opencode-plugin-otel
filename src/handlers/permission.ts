...
type PendingPermissionSpan =
  HandlerContext["pendingPermissionSpans"] extends Map<string, infer V>
    ? V
    : never;
...
function storePermissionSpan(
  ...
  const oldest =
    !ctx.pendingPermissionSpans.has(key) &&
    ctx.pendingPermissionSpans.size >= MAX_PENDING
...
const permissionHandlers = {
  asked(e: EventPermissionAsked, ctx: HandlerContext) {
    ...
    if (!request.tool) {
      return ctx.log(
        "debug",
        "otel: permission span skipped without tool correlation",
        {
    ...
    const tool = ctx.pendingToolSpans.get(
      `${request.sessionID}:${request.tool.callID}`
    );
    if (!tool) {
      return ctx.log(
        "warn",
        "otel: permission span skipped because tool span was not found",
        {
          sessionID: request.sessionID,
          requestID: request.id,
          callID: request.tool.callID,
      ...
      `${ctx.tracePrefix}permission.check`,
      {
        ...
        attributes: {
          ...
          "agent.type":
            ...
            (ctx.sessionParents.has(request.sessionID)
              ? "subagent"
              : "primary"),
  ...
  replied(e: EventPermissionReplied, ctx: HandlerContext) {
    ...
    if (reply.reply === "reject") {
      const tool = ctx.pendingToolSpans.get(
        `${reply.sessionID}:${pending.callID}`
      );
      if (tool) {
