...
function isolatedEnv(home: string, config: unknown, autoCompact: boolean) {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (normalized.startsWith("OPENCODE_") || normalized.startsWith("OTEL_")) {
      continue;
    }
...
export async function createE2EFixture(options: FixtureOptions) {
  ...
  return {
    ...
    async startRun(initialPrompt: string, timeoutMs = 45_000) {
      if (activeServer) {
        throw new Error("An OpenCode server run is already active");
      }
      const started = Date.now();
      ...
      try {
        await waitFor("OpenCode server startup", 15_000, async () => {
          if (proc.exitCode !== null) {
            throw new Error(
              `OpenCode server exited ${proc.exitCode} during startup`
            );
          }
        ...
        request = async (pathname: string, init?: RequestInit) => {
          ...
          try {
            ...
            throw new Error(
              `${init?.method ?? "GET"} ${url.pathname} failed: ${error instanceof Error ? error.message : String(error)}`
            );
          }
          if (response.ok) {
            return response;
          ...
          throw new Error(
            `${init?.method ?? "GET"} ${url.pathname} returned ${response.status}: ${await response.text()}`
          );
        };
        ...
        if (typeof session.id !== "string") {
          throw new Error(
            `Invalid session response: ${JSON.stringify(session)}`
          );
        }
      ...
      return {
        async steer(guidancePrompt: string) {
          if (steered || compacted) {
            throw new Error(
              "The active OpenCode run has already been advanced"
            );
          }
          try {
            await send(guidancePrompt);
            await waitFor(
              "the steering message to be stored",
              15_000,
              async () => {
        ...
        async finish(): Promise<RunResult> {
          if (!steered) {
            throw new Error(
              "Steer the active OpenCode run before finishing it"
            );
          }
          try {
        ...
        async manualCompact(): Promise<RunResult> {
          ...
          try {
            ...
            if ((await response.json()) !== true) {
              throw new Error("OpenCode did not confirm manual compaction");
            }
            await waitFor(
