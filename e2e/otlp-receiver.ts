...
function decodeValue(value: unknown): unknown {
  ...
  if (isRecord(value.arrayValue)) return records(value.arrayValue.values).map(decodeValue)
  if (isRecord(value.kvlistValue)) return decodeAttributes(value.kvlistValue.values)
  return value
}
...
export function startOtlpReceiver() {
  ...
  return {
    ...
    async waitForSpanCount(count: number, timeoutMs = 5_000) {
      ...
      while (spans().length < count && Date.now() < deadline) await Bun.sleep(20)
      return spans()
    },
