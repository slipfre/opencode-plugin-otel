      ...
      ? {
          usage: {
            ...
            ...(input.usage.cacheRead === undefined
              ? {}
              : { prompt_tokens_details: { cached_tokens: input.usage.cacheRead } }),
            ...(input.usage.reasoning === undefined
              ? {}
              : { completion_tokens_details: { reasoning_tokens: input.usage.reasoning } }),
          },
        }
      : {}),
  }
...
function response(reply: LlmReply) {
  ...
  if (reply.type === "text") {
    return sse([
      start,
      ...(reply.reasoning ? [chunk({ delta: { reasoning_content: reply.reasoning } })] : []),
...
export function startFakeLlm(replies: LlmReply[]) {
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (!next) return response({ type: "text", text: "unexpected request" })
      if (next.hold) await new Promise<void>(resolve => held.push(resolve))
