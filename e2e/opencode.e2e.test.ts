...
suite("OpenCode plugin E2E", () => {
  test("exports a complete text trace with token and reasoning attributes", async () => {
    ...
    try {
      ...
      expect(llm.attributes["llm.token_count.prompt_details.cache_write"]).toBe(
        0
      );
      expect(llm.attributes["llm.token_count.prompt_details.cache_read"]).toBe(
        4
      );
  ...
  test("keeps steered user input in one run with distinct interactions", async () => {
    ...
    try {
      ...
      const run = one(spans, "e2e.run");
      const interactions = spans.filter(
        (span) => span.name === "e2e.interaction"
      );
      ...
      expect(
        interactionByInput.get("initial request")?.attributes["output.value"]
      ).toBe("first response before steer");
      expect(
        interactionByInput.get("steering guidance")?.attributes["output.value"]
  ...
  test("keeps automatic compaction and continuation in one interaction trace", async () => {
    ...
    try {
      ...
      expect(
        BigInt(summary!.endTimeUnixNano) <= BigInt(compaction.endTimeUnixNano)
      ).toBe(true);
      expect(
  ...
  test("recovers a provider context overflow through automatic compaction", async () => {
    ...
    try {
      ...
      expect(
        typeof compaction.attributes["opencode.compaction.trigger_message.id"]
      ).toBe("string");
      ...
      ).toBe(compaction.attributes["opencode.compaction.trigger_message.id"]);
      expect(continuation!.attributes["opencode.llm.purpose"]).toBeUndefined();

      expect(
        BigInt(failed!.endTimeUnixNano) <= BigInt(compaction.startTimeUnixNano)
      ).toBe(true);
      expect(
        BigInt(compaction.startTimeUnixNano) <=
          BigInt(summary!.startTimeUnixNano)
  ...
  test("exports a foreground subtask trace with parent-child correlation", async () => {
    ...
    try {
      ...
      expect(
        llms.filter((span) => span.parentSpanId === parentInteraction!.spanId)
      ).toHaveLength(2);
      expect(
        llms.filter((span) => span.parentSpanId === childInteraction!.spanId)
      ).toHaveLength(1);
      expect(new Set(spans.map((span) => span.traceId)).size).toBe(1);
